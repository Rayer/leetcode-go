package leetcode

import (
	"testing"
)

//You are given a string num, representing a large integer. Return the largest-
//valued odd integer (as a string) that is a non-empty substring of num, or an
//empty string "" if no odd integer exists.
//
// A substring is a contiguous sequence of characters within a string.
//
//
// Example 1:
//
//
//Input: num = "52"
//Output: "5"
//Explanation: The only non-empty substrings are "5", "2", and "52". "5" is the
//only odd number.
//
//
// Example 2:
//
//
//Input: num = "4206"
//Output: ""
//Explanation: There are no odd numbers in "4206".
//
//
// Example 3:
//
//
//Input: num = "35427"
//Output: "35427"
//Explanation: "35427" is already an odd number.
//
//
//
// Constraints:
//
//
// 1 <= num.length <= 10⁵
// num only consists of digits and does not contain any leading zeros.
//
//
// Related Topics Math String Greedy 👍 1503 👎 88

// leetcode submit region begin(Prohibit modification and deletion)
func largestOddNumber(num string) string {
	var candidates []string
	var currentLength int

	for i, v := range num {
		if v%2 != 0 {
			if i < currentLength {
				continue
			}
			if i > currentLength {
				target := num[:i+1]
				candidates = []string{target}
				currentLength = i
			}
			if i == currentLength {
				target := num[:i+1]
				candidates = append(candidates, target)
			}
		}
	}
	// Handle candidates
	maxStr := ""
	for _, c := range candidates {

		if maxStr == "" {
			maxStr = c
		}

		//Handle if len is same
		for i := 0; i < len(c); i++ {
			if c[i] == maxStr[i] {
				continue
			}
			if c[i] > maxStr[i] {
				maxStr = c
				break
			}
		}
	}
	return maxStr
}

//leetcode submit region end(Prohibit modification and deletion)

func Test_LargestOddNumberInString(t *testing.T) {

}

func Test_LargestOddNumberInString_table(t *testing.T) {
	type args struct {
		num string
	}

	tests := []struct {
		name     string
		args     args
		expected string
	}{
		{
			name: "1",
			args: args{
				"52",
			},
			expected: "5",
		},
		{
			name: "2",
			args: args{
				"4206",
			},
			expected: "",
		},
		{
			name: "3",
			args: args{
				"35427",
			},
			expected: "35427",
		},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if res := largestOddNumber(test.args.num); res != test.expected {
				t.Errorf("expected %v but get %v", test.expected, res)
			}
		})
	}

}
